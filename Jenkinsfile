...
steps {
sh '''
ssh -i /var/lib/jenkins/.ssh/id_rsa -o StrictHostKeyChecking=no ec2-user@3.108.129.46  "
...
steps {
sh '''
ssh -i /var/lib/jenkins/.ssh/id_rsa -o StrictHostKeyChecking=no ec2-user@13.232.60.251 "
docker pull kongarashiva/react-app:${BUILD_NUMBER}
docker stop react-app || true
docker rm react-app || true
docker run -d --name react-app -p 3000:3000 kongarashiva/react-app:${BUILD_NUMBER}
